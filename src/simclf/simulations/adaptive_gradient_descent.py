...
from simclf.algorithm.gradient import calculate_gradient, index_tensor_excluding
...
class SimulationAdaptiveGradientDescent():
    ...
    def march(self, gradient: Point3DArray, step_size: float):
    ...
    def step(self, step_size: float):
        ...
        gradient_magnitudes = np.linalg.norm(self.gradient, axis=1)
        std = np.std(gradient_magnitudes)

        # Find the gradient for atoms with a lot of potential.
        # We'll march then until they settle down.
        temp_gradient = self.gradient.copy()
        for i in range(self.atom_positions.shape[0]):
            if gradient_magnitudes[i] < std * 1.75:
                temp_gradient[i] = (0.0, 0.0, 0.0)
        self.march(temp_gradient, step_size)

        # Find the gradient for atoms with less potential.
        # We'll march then until they settle down.
        temp_gradient = self.gradient.copy()
        for i in range(self.atom_positions.shape[0]):
            if gradient_magnitudes[i] >= std * 1.75:
...
def setup_and_run_simulation(
    ...
    simulation_dimension_scaler: float = 0.5,
):
    ...
    simulation_dimension = number_of_atoms**(1/3) * (2 * equilibrium_spacing) * simulation_dimension_scaler
    step_size = equilibrium_spacing / 100.0
    ...
    writer = Writer("adaptive_gradient_descent", "1.0")
    ...
    writer.write_object_to_json("config.json", {
        ...
        "simulation_dimension_scaler": simulation_dimension_scaler,
        "step_size": step_size,
    })
...
if __name__ == "__main__":
    ...
    # Controls how tightly the atoms are spaced when they are generated. >1 for more sparse. <1 for tighter.
    parser.add_argument("--simulation-dimension-scaler", "--simulation_dimension_scaler", dest="simulation_dimension_scaler", type=float)
